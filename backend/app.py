...
def register(
    ...
    if email and db.query(DBUser).filter(DBUser.email == email).first():
...
def admin_stats(
    ...
    verified_users = db.query(DBUser).filter(DBUser.is_verified == 1).count()
    admin_users = db.query(DBUser).filter(DBUser.is_admin == 1).count()
...
def toggle_admin(
    ...
    return {"message": f"User {'removed from' if user.is_admin == 0 else 'granted'} admin privileges"}
...
def admin_analytics(
    ...
    """Get scan analytics for admin dashboard."""
    from sqlalchemy import func

    total_scans = db.query(ScanHistory).count()
    phishing_scans = db.query(ScanHistory).filter(ScanHistory.prediction == "Phishing").count()
    legit_scans = db.query(ScanHistory).filter(ScanHistory.prediction == "Legitimate").count()
    ...
    for i in range(6, -1, -1):
        ...
        count = db.query(ScanHistory).filter(
            ScanHistory.timestamp >= day,
            ScanHistory.timestamp < next_day
        ).count()
        phish_count = db.query(ScanHistory).filter(
            ScanHistory.timestamp >= day,
            ScanHistory.timestamp < next_day,
        ...
        daily_data.append({
            ...
            "legitimate": count - phish_count
        })

    # Top scanned users
    top_users = (
        db.query(ScanHistory.user_id, func.count(ScanHistory.id).label("scan_count"))
    ...
    for user_id, count in top_users:
        user = db.query(DBUser).filter(DBUser.id == user_id).first()
        if user:
            top_users_data.append({"username": user.username, "scan_count": count})

    return {
        "total_scans": total_scans,
        "phishing_scans": phishing_scans,
        "legitimate_scans": legit_scans,
        "phishing_rate": round((phishing_scans / total_scans * 100), 1) if total_scans > 0 else 0,
        "daily_trend": daily_data,
        "top_users": top_users_data
    }


@app.get("/admin/users/{user_id}/scans")
def get_user_scans(
    user_id: int,
    admin_user: DBUser = Depends(get_admin_user),
    db: Session = Depends(get_db)
