...
function Auth({ setToken, setCurrentView, setUsername: setParentUsername, setIsAdmin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        AuthService.saveAuthData(data.access_token, username, profile?.is_admin);
        setToken(data.access_token);
        if (setParentUsername) setParentUsername(username);
        if (setIsAdmin) setIsAdmin(profile?.is_admin || false);
        setCurrentView('scanner');
      } else {
        // Register
        const data = await AuthService.register(username, password, email || undefined);
